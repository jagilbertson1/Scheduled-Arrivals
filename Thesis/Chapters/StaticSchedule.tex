...
\begin{algorithmic}
\Function{ObjectiveFunction}{$\mathbf{x}_{n}$,$\gamma$,$\mu$}
    \For {$i = 1, 2, \ldots, n$}
    	\For {$j = 0, 1, \ldots, (i - 1)$}
...
The constant interarrival simplification significantly reduces computation cost, but Figure~\ref{fig:Static_Time_Gamma} suggests that it is clearly not optimal for the first few and last few customers. Instead, Figure~\ref{fig:Static_Time_Num} suggests an alternative simplfication to reduce computation time.
\begin{figure}[htb]
	\centering
	\includegraphics[width = 0.85\textwidth]{Static_Line_Interarrival_Num.eps}
	\caption{Optimal interarrival time for each of the customers after the first customer. Figure generated assuming $\gamma = \frac{c_{S}}{c_{S} + c_{W}} = 0.5$ and $\mu = 1$. Each line is plotted for a different total number of customers. The lightest line is for $n = 15$. As $n$ decreases, the lines become darker.}
	\label{fig:Static_Time_Num}
\end{figure}

Figure~\ref{fig:Static_Time_Num} plots the optimal interarrival times $\mathbf{x}_{n}^{*} = (x_{1}^{*}, \ldots, x_{n - 1}^{*})$ for $n \in \{ 2, \ldots, 15 \}$ and $\gamma = 0.5$ fixed. All the plotted curves display the dome-shape as before. If we consider a fixed customer position, we observe that the optimal interarrival time $x_{i}^{*}$ converges reasonably quickly to a constant value as $n$ increases.

It may be possible to decompose the static problem into a series of subproblems that can be solved individually. Rather than solving for all of $\mathbf{x}_{1}^{*}, \ldots, \mathbf{x}_{15}^{*}$ separately, it might be possible to use the previously computed $\mathbf{x}_{1}^{*}, \ldots, \mathbf{x}_{i - 1}^{*}$ to efficiently solve for $\mathbf{x}_{i}^{*}$.
